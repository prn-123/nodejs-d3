...
Heros.update = function(newHeroData){
	...
	let query= `update comic set superhero ='${newHeroData.superhero}' ,publisher='${newHeroData.publisher}',alter_ego='${newHeroData.alter_ego}',first_appearance='${newHeroData.first_appearance}',characters='${newHeroData.characters}', is_valid=${newHeroData.is_valid}, 
	update_time='${new Date()}' where id = ${newHeroData.id}`;
	console.log(query)
	connection.query(query, function(err, result, fields){
		//it execute in  mysql and return the result;function() is a callback function 
			if(err){
